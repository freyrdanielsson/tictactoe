...
def mcarlo(board, player, n):
    monte = 0.0
    for i in range(0, n):
        winner = playRandom(np.copy(board), player)
        monte += winner
    return monte/n

def mc(board, player, n):
    moves = legal_moves(board)
    na = np.size(moves)
    va = np.zeros(na)
    for i in range(0,na):
        board[moves[i]] = player
        va[i] = mcarlo(np.copy(board), player, n)
        board[moves[i]] = 0
    return moves[np.argmax(va)]
...
def learnit(numgames, epsilon, alpha, monteNum = 10, debug = False):
    ...
    for games in range(0, numgames):
        ...
        for move in range(0, 9):
            ...
            if player == 1:
                ...
                action = epsilongreedy(np.copy(board), player, epsilon, debug) # Kemur breyting her? Monte Carlo if player 1?
            # Monte Carlo player
            if player == 2:
                action = mc(np.copy(board), player, monteNum)
